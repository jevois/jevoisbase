...
###################################################################################################
question "Download pre-compiled binaries for JeVois ${vjvber} instead of rebuilding from source"
if [ "X$REPLY" != "Xn" ]; then
    wget http://jevois.org/data/contrib-jevoisbase-binary-${jvbver}.tbz
    tar jxvf contrib-jevoisbase-binary-${jvbver}.tbz
    /bin/rm -f contrib-jevoisbase-binary-${jvbver}.tbz
    echo "Pre-compiled contribs done."
else
    ###################################################################################################
...
cd ..
###################################################################################################
question "Fetch and unpack contributed data for JeVois-A33"
if [ "X$REPLY" != "Xn" ]; then
...
###################################################################################################
question "Fetch and unpack contributed data for JeVois-Pro"
if [ "X$REPLY" != "Xn" ]; then
