...
#include <jevoisbase/src/Components/ObjectRecognition/ObjectRecognitionILAB.H>
...
void ObjectRecognitionILAB::define()
{
  ...
  int const n_fmaps = 10; // base number of feature maps
  ...
  (*net) << conv(32, 32, 5, 3, n_fmaps, tiny_dnn::padding::same)
         ...
         << conv(16, 16, 5, n_fmaps, n_fmaps * 2, tiny_dnn::padding::same)
         << pool(16, 16, n_fmaps * 2, 2)
         << conv(8, 8, 5, n_fmaps * 2, n_fmaps * 4, tiny_dnn::padding::same)
         << pool(8, 8, n_fmaps * 4, 2)
         << tiny_dnn::fully_connected_layer<tiny_dnn::activation::identity>(4 * 4 * n_fmaps * 4, n_fc)
         << tiny_dnn::fully_connected_layer<tiny_dnn::activation::softmax>(n_fc, n_categ);
}
