/*!@file env_image_ops.c Fixed-point integer math versions of some of our floating-point image functions */
...
void env_dec_y(const struct env_image* src, struct env_image* result)
{
  ...
  for (env_size_t j = 0; j < dims2.h; ++j)
  {
    for (env_size_t i = 0; i < dims2.w; ++i) dptr[i] = sptr[i];
...
void env_lowpass_5_x_dec_x(const struct env_image* src, const struct env_math* imath, struct env_image* result)
{
  ...
  env_c_lowpass_5_x_dec_x_fewbits_optim(env_img_pixels(src), w, h, env_img_pixelsw(result), dims2.w);
}
...
void env_lowpass_5_y_dec_y(const struct env_image* src, const struct env_math* imath, struct env_image* result)
{
  ...
  env_c_lowpass_5_y_dec_y_fewbits_optim(env_img_pixels(src), w, h, env_img_pixelsw(result), dims2.h);
}
...
void env_lowpass_9_x(const struct env_image* source, const struct env_math* imath, struct env_image* result)
{
  ...
  if (w < 9)  // use inefficient implementation for small images
  {
    ...
    for (env_size_t j = 0; j < h; ++j)
      for (env_size_t i = 0; i < w; ++i)
      {
        ...
        {
          if (i + k < hfs2 || i + k >= w + hfs2) continue;
          
          // convert to signed integers to avoid wraparound when k<hfs2
          val += src[(env_ssize_t) k - (env_ssize_t) hfs2] * hf_flipped[k];
          sum += hf_flipped[k];
        }
  ...
  env_c_lowpass_9_x_fewbits_optim(env_img_pixels(source), w, h, env_img_pixelsw(result));
}
...
void env_lowpass_9_y(const struct env_image* source, const struct env_math* imath, struct env_image* result)
{
  ...
  // if the height is less than 2, then the caller should handle that condition differently since no smoothing need be
  // done (so the caller could either copy or swap the source into the result location)
  ENV_ASSERT(h >= 2);
  ...
  if (h < 9)  // use inefficient implementation for small images
  {
    ...
    for (env_size_t j = 0; j < h; ++j)
      for (env_size_t i = 0; i < w; ++i)
      {
        ...
        {
          if (j + k < vfs2 || j + k >= h + vfs2) continue;
          
          // convert to signed integers to avoid wraparound when k<vfs2
          val += src[w * ((env_ssize_t) k - (env_ssize_t) vfs2)] * vf_flipped[k];
          sum += vf_flipped[k];
        }
  ...
  env_c_lowpass_9_y_fewbits_optim(env_img_pixels(source), w, h, env_img_pixelsw(result));
}
...
void env_lowpass_9(const struct env_image* src, const struct env_math* imath, struct env_image* result)
{
  ...
  if (tmp1.dims.h >= 2) env_lowpass_9_y(&tmp1, imath, result);
  else env_img_swap(&tmp1, result);
...
void env_quad_energy(const struct env_image* img1, const struct env_image* img2, struct env_image* result)
{
  ...
  for (env_size_t i = 0; i < sz; ++i)
  {
    ...
    /* "A Fast Approximation to the Hypotenuse" by Alan Paeth, from "Graphics Gems", Academic Press, 1990
       
       http://www.acm.org/pubs/tog/GraphicsGems/gems/HypotApprox.c

       gives approximate value of sqrt(s1*s1+s2*s2) with only overestimations, and then never by more than (9/8) + one
       bit uncertainty  */
...
// ######################################################################
void env_steerable_filter(const struct env_image* src, const intg32 kxnumer, const intg32 kynumer,
                          const env_size_t kdenombits, const struct env_math* imath, struct env_image* result)
{
  ENV_ASSERT(env_dims_equal(result->dims, src->dims));
  ...
  // let's do a conservative check to make sure that we won't overflow when we compute "arg" later on -- as a very rough
  // estimate, kxnumer and kynumer are on the order of 2^16 (8 bits from kdenombits=8, 8 bits from ENV_TRIG_TABSIZ=256),
  // which gives room for w+h to be up to about 2^15
...
void env_attenuate_borders_inplace(struct env_image* a, env_size_t size)
{
  ...
  for (env_size_t y = 0; y < dims.h; y ++)
  {
    ...
    {
      *(aptr + dims.w - 1 - x * 2) = (*(aptr + dims.w - 1 - x * 2) / size_plus_1) * coeff;
...
// ######################################################################
void env_pyr_build_hipass_9(const struct env_image* image, env_size_t firstlevel, const struct env_math* imath,
                            struct env_pyr* result)
{
  ...
  // special case for the zero'th pyramid level so that we don't have to make an extra copy of the input image at its
  // largest resolution
  ...
  if (0 == firstlevel)
  {
    env_img_resize_dims(env_pyr_imgw(result, 0), image->dims);
    env_c_image_minus_image(env_img_pixels(image), env_img_pixels(&lpfima), env_img_size(image),
  ...
  for (env_size_t lev = 1; lev < depth; ++lev)
  {
    ...
    if (lev >= firstlevel)
    {
      env_img_resize_dims(env_pyr_imgw(result, lev), dec.dims);
      env_c_image_minus_image(env_img_pixels(&dec), env_img_pixels(&lpfima), env_img_size(&dec),
...
// ######################################################################
void env_pyr_build_steerable_from_hipass_9(const struct env_pyr* hipass, const intg32 kxnumer, const intg32 kynumer,
                                           const env_size_t kdenombits, const struct env_math* imath,
                                           struct env_pyr* out)
{
  ...
  {
    // if the hipass is empty at a given level, then just leave the output empty at that level, too
    if (!env_img_initialized(env_pyr_img(hipass, lev))) continue;
    
    env_img_resize_dims(env_pyr_imgw(&result, lev), env_pyr_img(hipass, lev)->dims);
    
    env_steerable_filter(env_pyr_img(hipass, lev), kxnumer, kynumer, kdenombits, imath, env_pyr_imgw(&result, lev));

    // attenuate borders that are overestimated due to filter trunctation:
    env_attenuate_borders_inplace(env_pyr_imgw(&result, lev), attenuation_width);
  }
...
// ######################################################################
void env_pyr_build_lowpass_5(const struct env_image* image, env_size_t firstlevel, const struct env_math* imath,
                             struct env_pyr* result)
{
  ENV_ASSERT(env_img_initialized(image));
  ...
  for (env_size_t lev = 1; lev < depth; ++lev)
  {
    ...
    const struct env_image* prev = lev == 1 ? image : env_pyr_img(result, lev-1);
    
    env_lowpass_5_x_dec_x(prev, imath, &tmp1);
    env_lowpass_5_y_dec_y(&tmp1, imath, env_pyr_imgw(result, lev));
    
    if ((lev - 1) < firstlevel) env_img_make_empty(env_pyr_imgw(result, lev-1));
                            ...
                            const struct env_math* imath)
{
  for (env_size_t i = 0; i < depth; ++i)
  {
    ...
    }
    {
      ...
      if (src->dims.h >= 2) env_lowpass_9_y(src, imath, &tmp2);
      else env_img_swap(src, &tmp2);
      env_dec_y(&tmp2, src);
  ...
  */
  for (env_ssize_t j = 0; j < new_h; ++j)
  {
    ...
    for (env_ssize_t i = 0; i < new_w; ++i)
    {
      ...
      const intg32 dx0 = d00 + ((d10 - d00) / fx_denom) * fx_numer;
      const intg32 dx1 = d01 + ((d11 - d01) / fx_denom) * fx_numer;
...
// ######################################################################
void env_max_normalize_inplace(struct env_image* src, const intg32 mi, const intg32 ma,
                               const enum env_maxnorm_type normtyp, const intg32 rangeThresh)
{
  // do normalization depending on desired type:
  switch(normtyp)
  {
  case ENV_VCXNORM_NONE: env_max_normalize_none_inplace(src, mi, ma, rangeThresh); break;
  case ENV_VCXNORM_MAXNORM: env_max_normalize_std_inplace(src, mi, ma, rangeThresh); break;
  default: ENV_ASSERT2(0, "Invalid normalization type");
  }
}
...
void env_max_normalize_none_inplace(struct env_image* src, const intg32 nmi, const intg32 nma, const intg32 rangeThresh)
{
  ...
  intg32 ma = nma;
  if (mi != 0 || ma != 0)
    env_c_inplace_normalize(env_img_pixelsw(src), env_img_size(src), nmi, nma, &mi, &ma, rangeThresh);
}
...
{
  if (!env_img_initialized(src)) return;
  ...
  if (numlm > 1)
  {
    ...
    ENV_ASSERT((ma == lm_mean) || ((INTG32_MAX / (ma - lm_mean)) > (ma - lm_mean)));
...
// ######################################################################
void env_center_surround(const struct env_image* center, const struct env_image* surround,
                         const int absol, struct env_image* result)
{
  ...
  ENV_ASSERT2(lw >= sw && lh >= sh, "center must be larger than surround");
  ...
  // attenuate borders:
  env_attenuate_borders_inplace(result, ENV_MAX(result->dims.w, result->dims.h) / 20);
}

// ######################################################################
void env_get_rgby(const struct env_rgb_pixel* const src,  const env_size_t sz, struct env_image* rg,
                  struct env_image* by, const intg32 thresh,  const env_size_t inputbits)
{
...
void env_rescale_range_inplace(struct env_image* src, const intg32 mi, const intg32 ma)
{
  ...
  {
    for (env_size_t i = 0; i < sz; ++i) sptr[i] = 0;
  }
  else if ((INTG32_MAX / 255) > scale)
  {
    for (env_size_t i = 0; i < sz; ++i) sptr[i] = ((sptr[i] - mi) * 255) / scale;
  }
  else
  {
    const intg32 div = scale / 255;
    ENV_ASSERT(div > 0);
    for (env_size_t i = 0; i < sz; ++i) sptr[i] = (sptr[i] - mi) / div;
  }
}
...
{
  intg32 const * sptr = env_img_pixels(src);
  env_size_t const srcw = src->dims.w; env_size_t const tw = srcw / nx;
  env_size_t const srch = src->dims.h; env_size_t const th = srch / ny;
  env_size_t const stride = srcw - tw;
  env_size_t const ts = tw * th;
  ENV_ASSERT(tw > 0); ENV_ASSERT(th > 0);
  
  for (env_size_t j = 0; j < ny; ++j)
  {
    // Divide height de novo to avoid rounding errors if srch not multiple of ny:
    intg32 const * p = sptr + ((srch * j) / ny) * srcw;
    
    for (env_size_t i = 0; i < nx; ++i)
    {
      // Divide width de novo to avoid rounding errors if srcw not multiple of nx:
      intg32 const * pp = p + (srcw * i) / nx;
      
      intg32 sum = 0;
      for (env_size_t y = 0; y < th; ++y)
      {
        for (env_size_t x = 0; x < tw; ++x) sum += *pp++;
        pp += stride;
      }
...
// ######################################################################
void env_shift_clean(const struct env_image* srcImg, const env_ssize_t dx, const env_ssize_t dy,
                     struct env_image* result)
{
  ...
  if (ENV_ABS(dx) >= w || ENV_ABS(dy) >= h)
    // the shifts are so large that the resulting image will just be empty:
    return;
  ...
  {
    for (env_ssize_t i = startx; i < endx; ++i) *dst++ = *src++;
...
// ######################################################################
void env_shift_image(const struct env_image* srcImg, const env_ssize_t dxnumer, const env_ssize_t dynumer,
                     const env_size_t denombits, struct env_image* result)
{
  if (!env_img_initialized(srcImg))
  {
    env_img_make_empty(result);
  ...
  env_ssize_t xt = dxnumer >= 0 ? (dxnumer >> denombits) : - ((-dxnumer + denom-1) >> denombits);
  ...
  env_ssize_t yt = dynumer >= 0 ? (dynumer >> denombits) : - ((-dynumer + denom-1) >> denombits);
  ...
  // clear the return image
  {
    ...
    for (env_size_t i = 0; i < sz; ++i) rptr[i] = 0;
  }
  
  // dispatch to faster env_shift_clean() if displacements are roughly integer:
  ...
  if (xt > 0) ret2 += xt; else if (xt < 0) src2 -= xt;
  if (yt > 0) ret2 += yt * w; else if (yt < 0) src2 -= yt * w;
