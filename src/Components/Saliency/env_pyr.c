...
{
  pyr->images = (struct env_image*)env_allocate(n * sizeof(struct env_image));
  
  pyr->depth = n;
  
  for (env_size_t i = 0; i < pyr->depth; ++i) env_img_init_empty(&pyr->images[i]);
}

// ######################################################################
void env_pyr_make_empty(struct env_pyr* dst)
{
  for (env_size_t i = 0; i < dst->depth; ++i) env_img_make_empty(&dst->images[i]);
...
void env_pyr_copy_src_dst(const struct env_pyr* src, struct env_pyr* dst)
{
  ...
  if (dst->depth != src->depth)
  {
    for (env_size_t i = 0; i < dst->depth; ++i) env_img_make_empty(&dst->images[i]);
    env_deallocate(dst->images);
    dst->images = (struct env_image*)env_allocate(src->depth * sizeof(struct env_image));
    dst->depth = src->depth;
    for (env_size_t i = 0; i < dst->depth; ++i) env_img_init_empty(&dst->images[i]);
  }
  
  for (env_size_t i = 0; i < dst->depth; ++i) env_img_copy_src_dst(&src->images[i], &dst->images[i]);
}
